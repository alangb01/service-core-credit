...
public class ChargeCreditCardUseCase {
    ...
    public Single<Credit> execute(String creditId, BigDecimal amount) {
        ...
        return validateAmount(amount)
                ...
                .doOnSuccess(saved ->
                        log.info("[ChargeCreditCardUseCase] SUCCESS charge completed. creditId={}, customerId={}, finalBalance={}, finalAvailableBalance={}",
                                saved.id(), saved.customerId(), saved.balance(), saved.availableBalance())
                )
                .doOnError(error ->
                        log.error("[ChargeCreditCardUseCase] ERROR charge failed. creditId={}, amount={}, errorType={}, reason={}",
                        ...
                        Maybe.defer(() ->
                                repository.findById(creditId)
                                        ...
                                        .flatMap(credit ->
                                                cache.save(credit)
                                                        .onErrorComplete(e -> {
                                                            log.warn("[ChargeCreditCardUseCase] CACHE PUT ERROR (ignored). creditId={}, reason={}",
    ...
    private Single<Credit> validateCreditCard(Credit credit, BigDecimal amount) {
        ...
        if (credit.type() != CreditType.CREDIT_CARD) {
            log.warn("[ChargeCreditCardUseCase] RULE_VIOLATION credit type mismatch. creditId={}, expected=CREDIT_CARD, actual={}",
        ...
        if (credit.availableBalance().compareTo(amount) < 0) {
            log.warn("[ChargeCreditCardUseCase] LIMIT_EXCEEDED insufficient credit. creditId={}, availableBalance={}, requestedAmount={}",
        ...
        log.debug("[ChargeCreditCardUseCase] CREDIT UPDATED IN MEMORY. creditId={}, newBalance={}, newAvailableBalance={}",
