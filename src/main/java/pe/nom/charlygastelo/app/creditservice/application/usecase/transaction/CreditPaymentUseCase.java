...
public class CreditPaymentUseCase {
    ...
    public Completable execute(Transaction tx) {
        ...
        log.info("[CREDIT] Executing CREDIT_PAYMENT use case for txId={} amount={}",
                tx.id(), tx.amount());

        return Single.fromCallable(() -> {
                    tx.validateForPayment();
                    return tx;
                })
                .flatMap(transaction ->
                        creditRepository.findById(transaction.targetProductId())
                                .switchIfEmpty(Single.error(new BusinessRuleException("Credit not found")))
                                .map(credit -> credit.credit(transaction.amount()))
                                .flatMap(creditRepository::save)
                                .map(saved -> new ProcessedTransaction(transaction, saved, null))
                )
                .flatMapCompletable(processed ->
                        creditProducer.publishCreditPaymentOccurred(
                                processed.credit(),
                                processed.transaction()
                        )
                )
                .doOnComplete(() ->
                        log.info("[CREDIT] CREDIT_PAYMENT completed for txId={}", tx.id())
                )
                .onErrorResumeNext(err -> {
                    log.error("[CREDIT] CREDIT_PAYMENT failed for txId={} reason={}",
